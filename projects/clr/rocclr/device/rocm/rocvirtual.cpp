...
bool VirtualGPU::dispatchAqlPacketBatchFlat(const amd::AlignedVector64<uint8_t>& flatPacketData,
  ...
  uint8_t* metadataBase = nullptr;
  if (flatMetadataData != nullptr && !flatMetadataData->empty()) {
    if (flatMetadataData->size() != numPackets * kMetaPktSize) {
      return false;
    }
    if (metadata_preloader_.Active()) {
      metadataBase = static_cast<uint8_t*>(metadata_preloader_.QueueBase());
    }
  }
  ...
  for (size_t chunkStart = 0; chunkStart < numPackets; ) {
    ...
    // Copy pre-built metadata packets to the metadata ring (same slot indices).
    // Must complete before the sfence so metadata is visible when AQL headers go valid.
    if (metadataBase != nullptr) {
      const uint8_t* srcMeta = flatMetadataData->data() + chunkStart * kMetaPktSize;
      if (chunkSlot + thisChunk <= queueSize) {
        amd::nontemporalMemcpy(metadataBase + chunkSlot * kMetaPktSize, srcMeta,
                               thisChunk * kMetaPktSize);
      } else {
        const size_t firstCount = queueSize - chunkSlot;
        amd::nontemporalMemcpy(metadataBase + chunkSlot * kMetaPktSize, srcMeta,
                               firstCount * kMetaPktSize);
        amd::nontemporalMemcpy(metadataBase, srcMeta + firstCount * kMetaPktSize,
                               (thisChunk - firstCount) * kMetaPktSize);
      }
    }
...
bool VirtualGPU::submitKernelInternal(const amd::NDRangeContainer& sizes, const amd::Kernel& kernel,
  ...
  // Copy scheduler's AQL packet for possible relaunch from the scheduler itself
    ...
    if (isGraphCapture) {
      ...
      // Capture the matching metadata-prefetch packet (gfx1250) into a host buffer
      // parallel to the captured AQL packet.  Only built when this queue has a
      // metadata ring buffer; nothing is written to the queue here — the flattened
      // metadata is published to the metadata ring buffer at graph-launch time.
      if (metadata_preloader_.Active()) {
        if (uint8_t* metadataPacket = command_->getMetadataPacket(
                sizeof(hsa_amd_metadata_kernel_dispatch_packet_t))) {
          metadata_preloader_.CaptureMetadata(&dispatchPacket, aqlHeaderWithOrder,
                                              metadataPacket);
        }
      }
    } else {
